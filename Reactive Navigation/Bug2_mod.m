...
classdef Bug2_mod < Navigation
    ...
    properties(Access=protected)
        ...
        edge    % edge list
        k       % edge index
        myDirection  % Dirreccion del movimiento 0 Left 1 Right
    end

    methods

        function bug = Bug2_mod(varargin)
        ...
        function setMyDirection(bug, val, varargin)
            if val == 0
                bug.myDirection = 0;
            else
                bug.myDirection = 1;
        ...
        function n = next(bug, robot)
            ...
            if bug.step == 1
                ...
                if bug.isoccupied(robot + [dx; dy])
                    ...
                    if bug.myDirection == 0
                       bug.edge = edgelist(bug.occgridnav == 0, robot);
                    else
                        bug.edge = fliplr(edgelist(bug.occgridnav == 0, robot));
                    end
